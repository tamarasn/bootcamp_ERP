...
class TrainingCourse(models.Model):
    ...
    color = fields.Integer(string='Warna', default=get_default_color)
    email = fields.Char(string='Email', related='user_id.login')

    @api.constrains('name', 'description')
    def _check_fields_not_equal(self):
        for record in self:
            if record.name == record.description:
                raise ValidationError('Judul Kursus dan Keterangan tidak boleh sama!')
...
class TrainingSession(models.Model):
    ...
    def onchange_valid_duration(self):
        if self.duration <= 0:
            self.duration = 1
...
class TrainingAttendee(models.Model):
    ...
    session_ids = fields.Many2many('training.session', 'session_attendee_rel', 'attendee_id', 'session_id', string='Sesi')